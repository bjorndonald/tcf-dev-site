...
function FundingHeroSection (props) {
  ...
  const calculateTimeLeft = () => {
    const difference =
      +new Date(`12/31/2021 23:59:59 GMT-0800 (PST)`) - +new Date()
    let timeLeft = {}
    ...
    <div className='fundingSectionOne py-3 d-flex align-items-center justify-content-center'>
      <div className='container-fluid px-0'>
        <div className='text-center'>
          ...
          <div
            className='h4 countDownContainer position-relative mb-5 mb-lg-n4 text-white'
            style={{ zIndex: '100' }}
          >
            <span className='d-none d-md-block'>Ends in</span>
            <div className='countDownCircle mr-2 ml-md-2'>
              {timeLeft.days}
            </div>{' '}
            Days
            <div className='countDownCircle mx-2'>{timeLeft.hours}</div> Hours
            <div className='countDownCircle mx-2'>{timeLeft.minutes}</div>{' '}
            Minutes
          </div>
          <img
